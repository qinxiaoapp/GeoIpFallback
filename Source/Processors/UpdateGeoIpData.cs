...
namespace GeoIpFallback.Processorsv
{
    [UsedImplicitly]
    public class UpdateGeoIpData : StartTrackingProcessor
    {
        private const string ConfigKey = "GeoIpFallback.Mocks.Enabled\"";
        ...
        private bool IsMockEnabled
        {
            get
            {
                if (Tracker.Current.Session.Interaction.CustomValues.ContainsKey(ConfigKey))
                {
                    return (bool)Tracker.Current.Session.Interaction.CustomValues[ConfigKey];
                }

                var isEnabled = Settings.GetBoolSetting(ConfigKey, false);

                Tracker.Current.Session.Interaction.CustomValues.Add(ConfigKey, isEnabled);
        ...
        public override void Process(StartTrackingArgs args)
        {
            ...
            if (!Tracker.Current.Session.Interaction.HasGeoIpData)
            {
                ...
                {
                    Log.Debug($"Error resolving address: " + ex.Message, this);
                }
                catch (Exception ex)
                {
